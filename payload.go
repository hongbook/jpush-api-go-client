...
// SetBadgeClass 桌面图标对应的应用入口Activity类
func (n *AndroidNotification) SetBadgeClass(badgeClass string) *AndroidNotification {
	n.BadgeClass = badgeClass
	return n
}

// SetSound 填写Android工程中/res/raw/路径下铃声文件名称，无需文件名后缀
func (n *AndroidNotification) SetSound(sound string) *AndroidNotification {
	n.Sound = sound
	return n
}

// SetUriActivity 指定跳转页面
func (n *AndroidNotification) SetUriActivity(uriActivity string) *AndroidNotification {
	n.UriActivity = uriActivity
	return n
}

// SetUriAction 指定跳转页面
func (n *AndroidNotification) SetUriAction(uriAction string) *AndroidNotification {
